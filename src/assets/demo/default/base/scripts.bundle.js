  ...
  $.fn.mDatatable = function (options) {
    ...
    var Plugin = {
        ...
        if (args.table == $(datatable.wrap).attr('id')) {
          $(datatable).ready(function () {
            ...
            if ($(datatable.wrap).hasClass('m-datatable--subtable')) {
              $(datatable.tableBody)
                .find('.m-datatable__row:not(.m-datatable__row-detail):even')
        ...
        var scroll = {
          ...
          init: function () {
            ...
            if (options.layout.scroll) {
              ...
              if (
                $(scrollable).find('.m-datatable__row').length > 0 &&
                $(scrollable).length > 0
              ) {
                ...
                $(datatable.tableBody).find('.m-datatable__row').length > 0
              ) {
                scroll.scrollHead = $(datatable.tableHead).find(
                  '> .m-datatable__row'
                );
          ...
          mCustomScrollbar: function (scrollable) {
            ...
            if (
              Plugin.getOption('layout.smoothScroll.scrollbarShown') === true
            ) {
      ...
      dataRender: function (action) {
        ...
        var afterGetData = function (result) {
          var localPagingCallback = function (ctx, meta) {
            ...
            $(ctx.pager)
              .off()
              .on('m-datatable--on-goto-page', function (e) {
                $(ctx.pager).remove();
                ctx.init(meta);
              });
      ...
      getData: function () {
        ...
        return $.ajax(ajaxParams)
          .done(function (response, textStatus, jqXHR) {
            ...
            $(datatable).trigger('m-datatable--on-ajax-done', [
              datatable.dataSet
            ]);
          })
          .fail(function (jqXHR, textStatus, errorThrown) {
            Plugin.destroyScroller(
              $(datatable.table).find('.mCustomScrollbar')
            );
              ...
              .html(
                Plugin.dataPlaceholder(
                  Plugin.getOption('translate.toolbar.pagination.items.info'), {
                    ...
                    pageSize: pg.meta.perpage === -1 || pg.meta.perpage >= pg.meta.total ?
                      pg.meta.total :
                      pg.meta.perpage,
                    total: pg.meta.total
                  }
      ...
      columnHide: function () {
        ...
        $.each(options.columns, function (i, column) {
          if (typeof column.responsive !== 'undefined') {
            ...
            var tds = $.grep(
              $(datatable.table).find('.m-datatable__cell'),
              function (n, i) {
      ...
      setupSubDatatable: function () {
        ...
        if ($(datatable.table).find('.m-datatable__subtable').length > 0)
          return;
        ...
        var toggleSubTable = function (e) {
          ...
          if ($(parentRow).hasClass('m-datatable__row--subtable-expanded')) {
            ...
            $(datatable).trigger('m-datatable--on-collapse-subtable', [
              parentRow
            ]);
          } else {
            ...
            $(datatable).trigger('m-datatable--on-expand-subtable', [
              parentRow
            ]);
          }
      ...
      autoHide: function () {
          ...
          if (!$(detailRow).hasClass('m-datatable__row-detail')) {
            ...
            $(hidden).each(function () {
              ...
              $(detailSubTable).append(
                ...
                .append(
                  $('<td class="m-datatable__cell"></td>').append(
                    $('<span/>')
        ...
        $(datatable.tableBody)
          ...
          .each(function () {
            ...
            // check if subtable toggle exist
            if (
              $(datatable.tableHead).find('.m-datatable__toggle-detail')
              .length === 0
            ) {
              $(datatable.tableHead)
                .find('.m-datatable__row')
      ...
      filterArray: function (list, args, operator) {
        ...
        $.each(list, function (key, obj) {
          ...
          $.each(args, function (m_key, m_value) {
            if (
              to_match.hasOwnProperty(m_key) &&
              m_value == to_match[m_key].toLowerCase()
            ) {
              matched++;
            }
          });
      ...
      setActive: function (cell) {
        if (typeof cell === 'string') {
          ...
          cell = $(datatable.tableBody).find(
            '.m-checkbox--single > [type="checkbox"][value="' + cell + '"]'
          );
        }
      ...
      column: function (index) {
        Plugin.nodeCols = Plugin.recentNode = $(datatable.tableBody).find(
          '.m-datatable__cell:nth-child(' + (index + 1) + ')'
        );
        return datatable;
      },
      ...
      columns: function (selector) {
        ...
        var columns = $(context).find(
          '.m-datatable__cell[data-field="' + selector + '"]'
        );
        if (columns.length > 0) {
        ...
        if ($(Plugin.recentNode.length)) {
          if (bool) {
            if (Plugin.recentNode === Plugin.nodeCols) {
              Plugin.setOption(
                'columns.' + Plugin.recentNode.index() + '.responsive', {}
              );
            }
    ...
    if (typeof options !== 'undefined') {
      if (typeof options === 'string') {
        ...
        if (
          !datatable.data('mDatatable') &&
          !$(this).hasClass('m-datatable--loaded')
        ) {
          datatable.dataSet = null;
          datatable.textAlign = {
      ...
      init: function (options) {
        ...
        if (example.scrollable.length > 0) {
          if (example.scrollable.data('data-min-height')) {
            example.options.minHeight = example.scrollable.data(
              'data-min-height'
            );
          }

          if (example.scrollable.data('data-max-height')) {
            example.options.maxHeight = example.scrollable.data(
              'data-max-height'
            );
          }
        }
  ...
  $.fn.mHeader = function (options) {
    ...
    var Plugin = {
      ...
      toggle: function () {
        ...
        if (
          header.options.minimize.mobile === false &&
          header.options.minimize.desktop === false
        ) {
          return;
        }
        ...
        $(window).scroll(function () {
          ...
          if (
            (mUtil.isInResponsiveRange('tablet-and-mobile') &&
              header.options.classic &&
              header.options.classic.mobile) ||
            (mUtil.isInResponsiveRange('desktop') &&
              header.options.classic &&
              header.options.classic.desktop)
          ) {
            if (st > offset) {
              // down scroll mode
              $('body').addClass(on);
...
(function ($) {
    ...
    var Plugin = {
      ...
      build: function () {
        ...
        if (
          ...
          element.on(
            'click',
            '[data-menu-submenu-toggle="click"] > .m-menu__toggle, [data-menu-submenu-toggle="click"] > .m-menu__link .m-menu__toggle',
            Plugin.handleSubmenuDropdownClick
          );
          element.on(
            'click',
            '[data-menu-submenu-toggle="tab"] > .m-menu__toggle, [data-menu-submenu-toggle="tab"] > .m-menu__link .m-menu__toggle',
      ...
      reset: function () {
        ...
        menu
          .find('.m-menu__item--hover:not(.m-menu__item--tabs)')
      ...
      getSubmenuMode: function () {
        if (mUtil.isInResponsiveRange('desktop')) {
          ...
          mUtil.isset(menu.options.submenu, 'tablet')
        ) {
          return menu.options.submenu.tablet;
        } else if (
          mUtil.isInResponsiveRange('mobile') &&
          mUtil.isset(menu.options.submenu, 'mobile')
        ) {
      ...
      isConditionalSubmenuDropdown: function () {
        if (
          mUtil.isInResponsiveRange('desktop') &&
          mUtil.isset(menu.options.submenu, 'desktop.state.body')
        ) {
          return true;
        } else {
          return false;
      ...
      handleSubmenuDropdownClose: function (e, el) {
        ...
        if (
          shown.length > 0 &&
          el.hasClass('m-menu__toggle') === false &&
          el.find('.m-menu__toggle').length === 0
        ) {
      ...
      handleSubmenuAccordion: function (e, el) {
        ...
        if (
          Plugin.getSubmenuMode() === 'dropdown' &&
          item.closest('.m-menu__item').data('menu-submenu-mode') != 'accordion'
        ) {
          e.preventDefault();
          return;
        }
        ...
        if (
          item.closest('.m-menu__item').hasClass('m-menu__item--open-always')
        ) {
          return;
        }
      ...
      scrollToItem: function (item) {
        ...
        if (
          mUtil.isInResponsiveRange('desktop') &&
          menu.options.accordion.autoScroll &&
          !element.data('menu-scrollable')
        ) {
      ...
      showSubmenuDropdown: function (item) {
        ...
        element
          ...
          .each(function () {
            ...
            if (
              item.is(el) ||
              el.find(item).length > 0 ||
              item.find(el).length > 0
            ) {
              return;
            } else {
        ...
        // handle auto scroll for accordion submenus
        if (
          Plugin.getSubmenuMode() === 'accordion' &&
          menu.options.accordion.autoScroll
        ) {
      ...
      resize: function (e) {
        ...
        var itemsNumber =
          element.find('> .m-menu__nav > .m-menu__item').length - 1;
        var check;

        if (
          Plugin.getSubmenuMode() == 'dropdown' &&
          ((mUtil.isInResponsiveRange('desktop') &&
              mUtil.isset(menu.options, 'resize.desktop') &&
              (check = menu.options.resize.desktop) &&
              currentWidth <=
              (breakpoint = resize.data('menu-resize-desktop-breakpoint'))) ||
            (mUtil.isInResponsiveRange('tablet') &&
              mUtil.isset(menu.options, 'resize.tablet') &&
              (check = menu.options.resize.tablet) &&
              currentWidth <=
              (breakpoint = resize.data('menu-resize-tablet-breakpoint'))) ||
            (mUtil.isInResponsiveRange('mobile') &&
              mUtil.isset(menu.options, 'resize.mobile') &&
              (check = menu.options.resize.mobile) &&
              currentWidth <=
              (breakpoint = resize.data('menu-resize-mobile-breakpoint'))))
        ) {
          var moved = submenu.find('> .m-menu__subnav > .m-menu__item').length; // currently move
          var left = element.find(
            '> .m-menu__nav > .m-menu__item:not(.m-menu__item--resize)'
          ).length; // currently left
            ...
            if (moved > 0) {
              submenu
                ...
                .each(function () {
                  ...
                  var elementsNumber = submenu.find(
                    '> .m-menu__nav > .m-menu__item:not(.m-menu__item--resize)'
                  ).length;
                  element
                    .find(
      ...
      adjustSubmenuDropdownArrowPos: function (item) {
        ...
        if (arrow.length > 0) {
          ...
          if (
            submenu.hasClass('m-menu__submenu--classic') ||
            submenu.hasClass('m-menu__submenu--fixed')
          ) {
            ...
            if (
              submenu.hasClass('m-menu__submenu--center') ||
              submenu.hasClass('m-menu__submenu--full')
            ) {
              pos =
                item.offset().left -
                (mUtil.getViewPort().width - submenu.outerWidth()) / 2;
  ...
  $(document).on('click', function (e) {
    $(
      '.m-menu__nav .m-menu__item.m-menu__item--submenu.m-menu__item--hover:not(.m-menu__item--tabs)[data-menu-submenu-toggle="click"]'
    ).each(function () {
      var element = $(this)
        .closest('.m-menu__nav')
        .parent();
      ...
      if (
        ...
        element.find($(e.target)).length == 0
      ) {
        var items = element.find(
          '.m-menu__item--submenu.m-menu__item--hover:not(.m-menu__item--tabs)[data-menu-submenu-toggle="click"]'
        );
        items.each(function () {
      ...
      init: function (options) {
        ...
        if (messenger.scrollable.length > 0) {
          if (messenger.scrollable.data('data-min-height')) {
            messenger.options.minHeight = messenger.scrollable.data(
              'data-min-height'
            );
          }

          if (messenger.scrollable.data('data-max-height')) {
            messenger.options.maxHeight = messenger.scrollable.data(
              'data-max-height'
            );
          }
        }
...
(function ($) {
  ...
  $.fn.mPortlet = function (options) {
    ...
    var Plugin = {
      ...
      build: function () {
        // remove
        var remove = element.find(
          '> .m-portlet__head [data-portlet-tool=remove]'
        );
        if (remove.length === 1) {
        ...
        // reload
        var reload = element.find(
          '> .m-portlet__head [data-portlet-tool=reload]'
        );
        if (reload.length === 1) {
        ...
        var toggle = element.find(
          '> .m-portlet__head [data-portlet-tool=toggle]'
        );
        if (toggle.length === 1) {
      ...
      remove: function () {
        ...
        if (
          $('body').hasClass('m-portlet--fullscreen') &&
          element.hasClass('m-portlet--fullscreen')
        ) {
      ...
      setupTooltips: function () {
        if (portlet.options.tooltips) {
          var collapsed =
            element.hasClass('m-portlet--collapse') ||
            element.hasClass('m-portlet--collapsed');
          var fullscreenOn =
            $('body').hasClass('m-portlet--fullscreen') &&
            element.hasClass('m-portlet--fullscreen');
  ...
  $.fn.mQuicksearch = function (options) {
    ...
    var Plugin = {
      ...
      handleSearch: function (e) {
        ...
        $.ajax({
          ...
          error: function (res) {
            ...
            Plugin.handleCancelIconVisibility('on');
            qs.dropdown
              .setContent(qs.options.templates.error.apply(qs, res))
              .show();
            element.addClass(qs.options.hasResultClass);
          }
        });
      ...
      showDropdown: function (e) {
        if (
          qs.dropdown.isShown() == false &&
          qs.input.val().length > qs.options.minLength &&
          qs.processing == false
        ) {
          qs.dropdown.show();
          e.preventDefault();
    ...
    var Extension = {
      ...
      initSelect: function () {
        ...
        if (Extension.selectedAllRows && options.vars.requestIds) {
          ...
          if (
            !datatable.hasClass('m-datatable--error') &&
            $(datatable.tableBody)
            .find('.m-checkbox--single > [type="checkbox"]')
...
var mLayout = (function () {
    ...
    horMenu = $('#m_header_menu').mMenu({
      ...
      resize: {
        desktop: function () {
          ...
          if (
            headerMenuWidth + headerTopbarWidth + spareWidth >
            headerNavWidth
          ) {
            return false;
          } else {
  ...
  var initLeftAsideToggle = function () {
    var asideLeftToggle = $('#m_aside_left_minimize_toggle')
      .mToggle({
        target: 'body',
  ...
  var initMessages = function () {
    ...
    var init = function () {
      var height =
        ...
        messenger.find('.m-messenger__form').outerHeight(true) -
        120;
...
function copyToClipboard(__id) {
  var copyText = document.getElementById(__id);
  ...
  toastr['success']('Copied ' + textArea.value , '');
  textArea.remove();
}